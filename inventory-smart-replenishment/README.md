...
- 起发量下限：底表的 `起发量/最低起发量` 作为最低起订量独立生效——某仓要下单但取整结果低于起发量时，自动抬到对齐整装的最低起订量（不再被静默忽略）。
- 缺口偏小：某仓库存天数低于预警线、但在就近取整/起补门槛/在途扣减后建议补货量归零时，状态显示「缺口偏小」（而非误导性的「需补货 0 双」），且不进入补货提醒与图片报表。
